...
//    }
    ...
    FilterInitConfig_t filter_config = {
        ...
        .cutoff_freq = 50.0f,          // 默认截止频率，后续分通道覆盖
    ...
    for (int i = 0; i < 3; i++) {
        ...
        gyro_lowpass_filters[i] = LowpassFilter_Register(&filter_config);

        // 检查内存分配是否成功
        if (accel_moving_filters[i] == NULL || gyro_moving_filters[i] == NULL ||
            accel_notch_92_filters[i] == NULL || accel_notch_184_filters[i] == NULL ||
            gyro_notch_92_filters[i] == NULL || gyro_notch_184_filters[i] == NULL ||
            accel_lowpass_filters[i] == NULL || gyro_lowpass_filters[i] == NULL) {
            Error_Handler();
        }
    }
    ...
    float32_t notch_accel_92[3] = {0};      // 92Hz陷波后的加速度数据
    float32_t notch_accel_184[3] = {0};     // 184Hz陷波后的加速度数据
    float32_t notch_gyro_92[3] = {0};       // 92Hz陷波后的陀螺仪数据
    float32_t notch_gyro_184[3] = {0};      // 184Hz陷波后的陀螺仪数据
    float32_t moving_accel[3] = {0};        // 滑动平均后的加速度数据
    float32_t moving_gyro[3] = {0};         // 滑动平均后的陀螺仪数据
  ...
  for(;;)
  {
        ...
        if (bmi088_test != NULL) {
            ...
            // 数据读取与预处理
            for (int i = 0; i < 3; i++) {
                NotchFilter_Process(accel_notch_92_filters[i], bmi088_test->accel[i], &notch_accel_92[i]);
                NotchFilter_Process(accel_notch_184_filters[i], notch_accel_92[i], &notch_accel_184[i]);
                MovingAvgFilter_Process(accel_moving_filters[i], notch_accel_184[i], &moving_accel[i]);
                LowpassFilter_Process(accel_lowpass_filters[i], moving_accel[i], &filtered_accel[i]);
            }
        ...
        //     FusionAhrsInternalStates states = FusionAhrsGetInternalStates(&fusion_ahrs);
            ...
            for (int i = 0; i < 3; i++) {
                NotchFilter_Process(gyro_notch_92_filters[i], raw_gyro.array[i], &notch_gyro_92[i]);
                NotchFilter_Process(gyro_notch_184_filters[i], notch_gyro_92[i], &notch_gyro_184[i]);
                MovingAvgFilter_Process(gyro_moving_filters[i], notch_gyro_184[i], &moving_gyro[i]);
                LowpassFilter_Process(gyro_lowpass_filters[i], moving_gyro[i], &filtered_gyro[i]);
            }
